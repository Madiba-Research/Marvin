...
def finder(root, transform=str, nlist=None, count=[-1], window_hash="", memory=None):
    ...
    if nlist is None:
        nlist = []
    if not root or count[0] == 0 or (memory.get(window_hash) != None and (root.getUniqueId()+root.getId()+str(root.isClickable())+str(root.__getattr__('isEnabled')())+"-second") in memory.get(window_hash)):
        return
    hhash = (root.getUniqueId()+root.getId()+str(root.isClickable())+str(root.__getattr__('isEnabled')()))
    if transform(root) and (window_hash == "" or memory.get(window_hash) == None or hhash not in memory.get(window_hash)):
        if count[0] != -1:
            count[0] -= 1
        nlist.append(root)
        if window_hash != "":
    ...
    if window_hash in memory and hhash in memory.get(window_hash) and hhash+"-second" not in memory.get(window_hash):
        memory.get(window_hash).add(root.getUniqueId()+root.getId()+str(root.isClickable())+str(root.__getattr__('isEnabled')())+"-second")
...
def get_root(device, window=-1, sleep=0.1):
...
class GooglePlay():
    ...
    def install(self, package):
        if requests.get("https://play.google.com/store/apps/details?id=" + package).status_code == 404:
            print(f"ERROR::{package}: 404")
            with open("out/"+package+".fail", "w") as f:
                f.write("404")
            return None
        print(f"Start to download: {package}")
...
class App:
    ...
    def smart(self,cert_type,dynamic_analysis, duration=300):
        futile = -1
        back_key = 2
        app_closed = 0
        time.sleep(3)
        ...
        for _ in range(0, 100):
            ...
            if os.path.exists("./skip"):
                os.remove("./skip")
                break
            if int(time.time()) - start > duration:  # 300
                break
            if self.d.is_app_crashed(self.p):
                with open("out/"+self.p+"/crash-"+self.stage+"-"+cert_type+"-"+str(dynamic_analysis)+"-"+str(int(time.time()))+".txt", "a") as f:
            ...
            if self.d.is_app_hangs(self.p):
                with open("out/"+self.p+"/hang-"+self.stage+"-"+cert_type+"-"+str(dynamic_analysis)+"-"+str(int(time.time()))+".txt", "a") as f:
                    f.write(str(int(time.time()))+"\n")
                    self.d.shell('input keyevent KEYCODE_BACK')
            # print("debug:2")
            if self.d.frida.is_lost != 0: # and dynmaic was off
                print("Kill here 4")

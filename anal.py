            ...
            for _encode_type,part in itemTransformer(_part).items():
                ...
                for p in [part[i:i+n] for i in range(0, len(part), n)]:
                    if (len(p) > 32 and p in whole):
                        if not all(c == 'A' or c == '=' for c in p) :#and b"\x00\x00\x00\x00" not in p:
                            return True
    return False
...
def cryptoAPI(crypt, crypts, keys, stage):
    ...
    if res["method_name"] == "doFinal":
        ...
        if crypt_len == 0:
            ...
            res["encrypt"] = res["encrypt"] if res["encrypt"] != None else (True if has_printable(_input) else False if has_printable(_output) else None)
            if res["encrypt"]:
                _input += base64.b64decode(crypt["args"][0])                
            else:
...
def read_whole_files(app_path):
    whole_files = dict()
    for root, _dirs, dump_files in os.walk(app_path+"/files-1"):
        for name in dump_files:
            file_path = os.path.join(root, name)
            with open(file_path, "rb") as f:
                whole_files[file_path[len(app_path):]] = f.read()

    for root, _dirs, dump_files in os.walk(app_path+"/files-2"):
        for name in dump_files:
            file_path = os.path.join(root, name)
            with open(file_path, "rb") as f:
                whole_files[file_path[len(app_path):]] = f.read()
...
def read_files_buffer(app_path):
    files_buffer = dict()
    if os.path.exists(app_path+"/fs-1.txt"):
        fs_1 = [json.loads(line.strip())
                for line in open(app_path+"/fs-1.txt", "r").readlines()]
        for fs in fs_1:
    ...
    if os.path.exists(app_path+"/fs-2.txt"):
        fs_1 = [json.loads(line.strip())
                for line in open(app_path+"/fs-2.txt", "r").readlines()]
        for fs in fs_1:
    ...
    for crypt_outter in cryptApi:
        ...
        for crypt_inner in cryptApi:
            if (crypt_inner["ciphertext"] == crypt_outter["plaintext"] or crypt_inner["ciphertext"] == extract(crypt_outter["plaintext"])) and crypt_inner["3rd"] == False and crypt_inner["plaintext"] != crypt_outter["ciphertext"]  and crypt_inner["encrypt"] == crypt_outter["encrypt"] and not all(0 == char for char in crypt_inner["plaintext"]) and crypt_inner["stage"] == crypt_outter["stage"] and not (crypt_inner["algorithm"] == None and crypt_inner["3rd"] == False):
                ...
                nestedApi = [(incApi,ncApi) for incApi,ncApi in enumerate(newcryptApi) if ncApi["ciphertext"] == crypt_outter["ciphertext"] and ncApi["ts"] == crypt_outter["ts"]]
                if len(nestedApi) == 0:
...
def extract_packets(pcap_file, data):
    packets = dict()
    raw_packets = dpkt.pcap.Reader(open(pcap_file, "rb"))
    for _, pkt in raw_packets:
        try:
            packet = dpkt.ethernet.Ethernet(pkt)
        except:
            print(f"ERROR:::dpkt.ethernet.Ethernet {p}, {e}") 
            continue
...
def extract_packets(pcap_file, data):
    ...
    try:
        ...
        print(f"ERROR:::dpkt.NeedData {p}, {e}")    
    return packets
'''


def extract_non_http_packets(packets):
...
'''
http:
request: pkg, host, path, port, scheme, method, port,  headers, timestamp_start
response: pkg, host, path, status_code, content-type, cookie, timestamp_start
'''
    ...
    with open(mitm_file, "rb") as logfile:
        ...
        try:
            for flow in freader.stream():
                ...
                req_dict = dict()
                req_dict["pkg"] = app
                req_dict["host"] = request.host
                req_dict["path"] = request.path
                req_dict["port"] = request.port
                req_dict["scheme"] = request.scheme
                req_dict["method"] = request.method
                req_dict["timestamp"] = request.timestamp_start
                # hdr = json.dumps(request.headers)
                # req_dict["headers"] = hdr
                ...
                if flow.response is not None:
                    ...
                    resp_dict = {}
                    # resp_dict["headers"] = hdr
                    # resp_dict["cookies"] = cookies
                    resp_dict["pkg"] = app
                    resp_dict["status_code"] = response.status_code
                    resp_dict["timestamp"] = response.timestamp_start
                    responses_list.append(resp_dict)
        except FlowReadException as e:
            print(f"Flow file corrupted: {e}")
            pass
        try:
            with open(app_path+"/requests.json", "w") as f:
                json.dump(requests_list, f)
        except FileNotFoundError as e:
            print(f"ERROR: {e}")

        try:
            with open(app_path+"/responses.json", "w") as f:
                json.dump(responses_list, f)
        except FileNotFoundError as e:
            print(f"ERROR: {e}")


def extract_raw_crypt_file(app_path, cryptApi, non_http_crypt, http_crypt, https_crypt, whole_files_crypt, files_buffer_crypt):
    raw_crypt_file = dict()
    if cryptApi:
        dst_folder = app_path+"/everything.crypt"
        os.makedirs(dst_folder)
        crypt_file_details = []
        for _i,crypt in enumerate(cryptApi):
            plaintext = extract(crypt["plaintext"])
            _path = str(crypt["stage"])+("-e-" if crypt["encrypt"] else "-d-")+("nested-" if "nested" in crypt else "normal-")+str(crypt["ts"])+"-"+str(_i)
    ...
    if non_http_crypt:
        dst_folder = app_path+"/non_http.crypt"
        os.makedirs(dst_folder)
        crypt_file_details = []
    ...
    if http_crypt:
        dst_folder = app_path+"/http.crypt"
        os.makedirs(dst_folder)
        crypt_file_details = []
    ...
    if https_crypt:
        dst_folder = app_path+"/https.crypt"
        os.makedirs(dst_folder)
        crypt_file_details = []
        for _i, (m, c) in enumerate(https_crypt):
            plaintext = extract(c["plaintext"])
            _path = str(c["stage"])+("-e-" if c["encrypt"] else "-d-")+("nested-" if "nested" in c else "normal-")+str(c["ts"])+"-"+str(_i)
            f_path = dst_folder+"/"+_path
            with open(f_path, "ab") as n_c:
                n_c.write(plaintext)
            crypt_file_details.append("{path}@{stage}:{algorithm}:{key}:{iv}:{thrid_party}{direction}{addr}".format(path=_path,stage=c["stage"], algorithm=(c["algorithm"] if c["algorithm"] else ""), key=(
    ...
    if whole_files_crypt:
        dst_folder = app_path+"/whole_files.crypt"
        os.makedirs(dst_folder)
        crypt_file_details = []
    ...
    if files_buffer_crypt:
        dst_folder = app_path+"/files_buffer.crypt"
        os.makedirs(dst_folder)
        crypt_file_details = []
    ...
    return raw_crypt_file


def delete_files_after_analysis(app_path):
    for fname in ("files_buffer.crypt","whole_files.crypt","non_http.crypt","http.crypt","https.crypt","everything.crypt","encrypt.xxx","decrypt.xxx","non_http_encrypt.xxx","non_http_decrypt.xxx","http_encrypt.xxx","http_decrypt.xxx","https_encrypt.xxx","https_decrypt.xxx","files_encrypt.xxx","files_decrypt.xxx","files_buffer_encrypt.xxx","files_buffer_encrypt.xxx","nested_encrypt.xxx","nested_decrypt.xxx","nested_non_http_encrypt.xxx","nested_non_http_decrypt.xxx","nested_http_encrypt.xxx","nested_http_decrypt.xxx","nested_https_encrypt.xxx","nested_https_decrypt.xxx","nested_files_encrypt.xxx","nested_files_decrypt.xxx","nested_files_buffer_encrypt.xxx","nested_files_buffer_encrypt.xxx","nested_crypt"):
        fullname = app_path+"/"+fname
...
def report(full_path, data,base_path="./"):
    ...
    cryptApi = create_cryptApi(path, app)

    # < files_buffer
    files_buffer = read_files_buffer(path+app)

    # < whole_files start
    whole_files = read_whole_files(path+app)
    # end whole_files

    # <start pcap
    pcap = path+app+"/"+app+".pcap"
    if os.path.exists(pcap):
        packets = extract_packets(pcap, data)
    # end pcap>

    # <start non-http
    if None != packets: 
        non_http_packets = extract_non_http_packets(packets)
    # end non-http>

    # <start http
    if os.path.exists(path+app+"/mitmdump"):
        normal_http, normal_https = extract_normal_http_https(path+app+"/mitmdump")
        extract_insecure_https(path+app, app, path+app+"/mitmdump")
    # end http>
    ...
    delete_files_after_analysis(path+app)
    
    raw_crypt_file = dict()
    raw_crypt_file = extract_raw_crypt_file(path+app, cryptApi, non_http_crypt, http_crypt, https_crypt, whole_files_crypt, files_buffer_crypt)
    ...
    with open(path+app+'/raw_crypt.json', 'w') as rcoutfile:
...
def finder(data, packets, used_keys, t, path):
    ...
    for raw in final_raw_data:
        ...
        extracted_raw_data = extracted_raw_data if type(extracted_raw_data) == bytes else extracted_raw_data.encode("utf-8")
                ...
                for lv_data in (value_data if type(value_data) == list else [value_data]):
                    ...
                    if name_data.endswith("-word"):
                        ...
                        if (lv_data in meta.lower() and name_data != "password-word") or (meta.lower().endswith(b">"+lv_data) and name_data == "password-word"):
                            item_result = (lv_data+b"%"+base64.b64encode(extracted_raw_data)).decode()
                    ...
                    if len(item_result) > 2:
                        if "meta" in raw:
                            item_result += (b"*"+base64.b64encode(raw["meta"].encode("utf-8"))).decode()
                        if "crypt" in raw:
        ...
        for k, v in key_list:
            for _encode_type, cryptographyItem in itemTransformer(v).items():
                if cryptographyItem in extracted_raw_data:
                    ...
                    if "meta" in raw:
                        key_item_result += (b"*"+base64.b64encode(raw["meta"].encode("utf-8"))).decode()
...
if __name__ == "__main__":
    main()
    # path = '/home/wolf/Workspace/non-standard2/out/'
    # for app in os.listdir(path):
    #     if os.path.exists(path+app+"/mitmdump"):
    #         extract_insecure_https(path, app, path+app+"/mitmdump")

            ...
            onLeave: function (retval) {
                if (this.socketType == 'tcp' || this.socketType == 'udp' || this.socketType == 'tcp6' || this.socketType == 'udp6') {
                    ...
                    if (JavaConnectionPool[(this.ipAddress + ':' + this.portAddress).replace(/^::ffff:/, '')] !== undefined && JavaConnectionPool[(this.ipAddress + ':' + this.portAddress).replace(/^::ffff:/, '')].includes((this.localIpAddress + ':' + this.localPortAddress).replace(/^::ffff:/, ''))) {
                        ...
                        var socketInfo = '"protocol":"' + this.socketType + '","fd":"' + this.fd + '","function":"' + ex.name + '","pid":' + Process.id + ',"address":"' + this.ipAddress + ':' + this.portAddress + '","local_address":"' + this.localIpAddress + ':' + this.localPortAddress;
                       printStackTraceNative(socketInfo, this.context, "java");
                    } else {
                        ...
                        var socketInfo = '"protocol":"' + this.socketType + '","fd":"' + this.fd + '","function":"' + ex.name + '","pid":' + Process.id + ',"address":"' + this.ipAddress + ':' + this.portAddress + '","local_address":"' + this.localIpAddress + ':' + this.localPortAddress;
                       printStackTraceNative(socketInfo, this.context, "native");
                    }
...
function printStackTraceNative(socketInfo, context, type) {
    var bt = Thread.backtrace(context, Backtracer.ACCURATE).map(DebugSymbol.fromAddress).join('\n').toString();
    var stacktrace = base64(bt);
    var ts = Date.now().toString();
    send('{"' + type + '":{' + socketInfo + '","stacktrace":"' + stacktrace +'","ts":"' + ts +'"}}');
}
